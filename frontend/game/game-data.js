const gameDefinitions = {
    ...
    eventos_casa_interrogacao: [
        ...
        },
        {
            ...
            efeito_detalhado:
                "Você tem 50% de chance de ganhar 10 moedas ou um item aleatório.",
        },
    ],
};
...
function getTipoCasaPelaCor(cor) {
    switch (cor) {
        case "blue":
            return "azul";
        case "red":
...
const mapaFinal = mapaBase.map((ponto) => {
    const pontoCor = pontosParaPintar.find(
        (p) => p.x === ponto.x && p.y === ponto.y
    );
